-- Email Pattern Intelligence System (Week 1 Day 1-2) - FINAL VERSION
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='email_patterns' AND column_name='last_source') THEN
    ALTER TABLE email_patterns ADD COLUMN last_source TEXT DEFAULT 'unknown';
  END IF;
...
-- Map old 'source' values to new 'last_source' values
UPDATE email_patterns SET last_source = CASE
  WHEN source = 'verified_pattern' THEN 'manual'
  WHEN source LIKE 'AI Pattern%' THEN 'llm'
  WHEN source = 'ai_pattern_found' THEN 'llm'
  WHEN source = 'smart_permutation' THEN 'nb'
  WHEN source = 'manual' THEN 'manual'
  WHEN source = 'llm' THEN 'llm'
  WHEN source = 'rag' THEN 'rag'
  WHEN source = 'rules' THEN 'rules'
  WHEN source = 'hybrid' THEN 'hybrid'
  ELSE 'unknown'
END
WHERE last_source = 'unknown' OR last_source IS NULL;

-- Update confidence to be between 0 and 1 if it's not already
UPDATE email_patterns SET confidence = CASE
  WHEN confidence > 1 THEN confidence / 100.0  -- Convert percentage to decimal
  ELSE confidence
END
WHERE confidence > 1;
...
COMMENT ON COLUMN email_patterns.last_source IS 'How pattern was discovered: rag/rules/llm/nb/hybrid/manual/unknown';

-- ═══════════════════════════════════════════════════════════════════════
-- STEP 4: Create indexes
-- ═══════════════════════════════════════════════════════════════════════

-- Vector similarity index (may take 1-2 minutes on large datasets)
...
-- ═══════════════════════════════════════════════════════════════════════
-- STEP 5: Create feedback loop table
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
-- STEP 6: Create domain health cache
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
-- STEP 7: Create enrichment telemetry
-- ═══════════════════════════════════════════════════════════════════════
...
SELECT
  ...
  ROUND(AVG(llm_cost_cents)::numeric, 4) as avg_llm_cost_cents,
  ROUND(AVG(nb_cost_cents)::numeric, 4) as avg_nb_cost_cents,
  ROUND(AVG(total_cost_cents)::numeric, 4) as avg_total_cost_cents,
...
SELECT 'email_patterns table',
       CASE WHEN COUNT(*) > 0 THEN '✅ Extended' ELSE '❌ Missing' END
...
SELECT 'Helper functions',
       CASE WHEN COUNT(*) >= 3 THEN '✅ Created (3)' ELSE '❌ Missing' END
FROM pg_proc WHERE proname IN ('increment_pattern_usage', 'calculate_pattern_success_rate', 'record_pattern_validation')
UNION ALL
SELECT 'Performance views',
       CASE WHEN COUNT(*) >= 3 THEN '✅ Created (3)' ELSE '❌ Missing' END
FROM pg_views WHERE viewname IN ('v_pattern_performance', 'v_enrichment_costs', 'v_pattern_feedback_summary');
