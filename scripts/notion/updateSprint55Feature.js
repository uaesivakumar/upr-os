#!/usr/bin/env node
/**
 * Update a Sprint 55 feature status in Notion
 * Usage: node updateSprint55Feature.js "Feature Name" "Status"
 */
...
async function updateFeature(featureName, newStatus) {
  try {
    ...
    if (response.results.length === 0) {
      console.log(`❌ Feature "${featureName}" not found in Sprint 55`);
      process.exit(1);
    }
...
if (!featureName || !newStatus) {
  console.log('Usage: node updateSprint55Feature.js "Feature Name" "Status"');
  console.log('Example: node updateSprint55Feature.js "Time-series forecasting service" "Done"');
  process.exit(1);
}
