#!/usr/bin/env node
/**
 * Get Sprint 45 tasks from Notion
 */
...
async function getSprint45Tasks() {
  console.log('🔍 Fetching Sprint 45 requirements from Notion...\n');

  try {
    // Get Sprint 45 details
    const sprintsResponse = await notion.databases.query({
      database_id: dbIds.sprints_db_id,
      filter: { property: 'Sprint', title: { equals: 'Sprint 45' } }
    });

    if (sprintsResponse.results.length === 0) {
      console.log('❌ Sprint 45 not found');
      process.exit(1);
    }
    ...
    console.log('📋 SPRINT 45 DETAILS:');
    console.log('='.repeat(80));

    // Sprint name
    const sprintName = sprintProps.Sprint?.title?.[0]?.text?.content || 'Sprint 45';
    console.log(`\nSprint: ${sprintName}`);
    ...
    // Get all tasks for Sprint 45
    console.log('\n' + '='.repeat(80));
    console.log('📝 SPRINT 45 TASKS:\n');
    ...
    console.log('='.repeat(80));
    console.log('\n✅ Sprint 45 requirements retrieved successfully\n');
