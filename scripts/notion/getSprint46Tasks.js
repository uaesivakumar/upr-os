#!/usr/bin/env node
/**
 * Get Sprint 46 tasks from Notion
 */
...
async function getSprint46Tasks() {
  console.log('🔍 Fetching Sprint 46 requirements from Notion...\n');

  try {
    // Get Sprint 46 details
    const sprintsResponse = await notion.databases.query({
      database_id: dbIds.sprints_db_id,
      filter: { property: 'Sprint', title: { equals: 'Sprint 46' } }
    });

    if (sprintsResponse.results.length === 0) {
      console.log('❌ Sprint 46 not found');
      process.exit(1);
    }
    ...
    console.log('📋 SPRINT 46 DETAILS:');
    console.log('='.repeat(80));

    // Sprint name
    const sprintName = sprintProps.Sprint?.title?.[0]?.text?.content || 'Sprint 46';
    console.log(`\nSprint: ${sprintName}`);
    ...
    // Get all tasks for Sprint 46
    console.log('\n' + '='.repeat(80));
    console.log('📝 SPRINT 46 TASKS:\n');

    const featuresResponse = await notion.databases.query({
      database_id: dbIds.module_features_db_id,
      filter: { property: 'Sprint', number: { equals: 46 } }
    });
    ...
    console.log('='.repeat(80));
    console.log('\n✅ Sprint 46 requirements retrieved successfully\n');
