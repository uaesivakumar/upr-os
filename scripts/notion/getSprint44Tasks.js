#!/usr/bin/env node
/**
 * Get Sprint 44 tasks from Notion
 */
...
async function getSprint44Tasks() {
  console.log('🔍 Fetching Sprint 44 requirements from Notion...\n');

  try {
    // Get Sprint 44 details
    const sprintsResponse = await notion.databases.query({
      database_id: dbIds.sprints_db_id,
      filter: { property: 'Sprint', title: { equals: 'Sprint 44' } }
    });

    if (sprintsResponse.results.length === 0) {
      console.log('❌ Sprint 44 not found');
      process.exit(1);
    }
    ...
    console.log('📋 SPRINT 44 DETAILS:');
    console.log('='.repeat(80));

    // Sprint name
    const sprintName = sprintProps.Sprint?.title?.[0]?.text?.content || 'Sprint 44';
    console.log(`\nSprint: ${sprintName}`);
    ...
    // Get all tasks for Sprint 44
    console.log('\n' + '='.repeat(80));
    console.log('📝 SPRINT 44 TASKS:\n');

    const featuresResponse = await notion.databases.query({
      database_id: dbIds.module_features_db_id,
      filter: { property: 'Sprint', number: { equals: 44 } }
    });
    ...
    console.log('='.repeat(80));
    console.log('\n✅ Sprint 44 requirements retrieved successfully\n');
