#!/usr/bin/env node
/**
 * Update a Sprint 51 feature status in Notion
 * Usage: node updateSprint51Feature.js "Feature Name" "Status"
 */
...
async function updateFeature(featureName, newStatus) {
  try {
    ...
    if (response.results.length === 0) {
      console.log(`❌ Feature "${featureName}" not found in Sprint 51`);
      process.exit(1);
    }
...
if (!featureName || !newStatus) {
  console.log('Usage: node updateSprint51Feature.js "Feature Name" "Status"');
  console.log('Example: node updateSprint51Feature.js "Design modern analytics dashboard" "Done"');
  process.exit(1);
}
