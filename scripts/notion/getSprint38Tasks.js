#!/usr/bin/env node
/**
 * Fetch Sprint 38 tasks from Notion
 */
...
async function getSprint38Tasks() {
  console.log('📋 Fetching Sprint 38 tasks from Notion...\n');

  try {
    // Get Sprint 38 info from Sprints DB
    ...
    if (sprintResponse.results.length === 0) {
      console.log('❌ Sprint 38 not found in Sprints database');
      process.exit(1);
    }

    const sprint = sprintResponse.results[0];
    const sprintName = sprint.properties.Sprint?.title?.[0]?.text?.content || 'Sprint 38';
    ...
    // Get all features for Sprint 38
    ...
    console.log(`📝 Found ${featuresResponse.results.length} tasks for Sprint 38:\n`);
