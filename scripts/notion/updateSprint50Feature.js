#!/usr/bin/env node
/**
 * Update a Sprint 50 feature status in Notion
 * Usage: node updateSprint50Feature.js "Feature Name" "Status"
 */
...
async function updateFeature(featureName, newStatus) {
  try {
    ...
    if (response.results.length === 0) {
      console.log(`❌ Feature "${featureName}" not found in Sprint 50`);
      process.exit(1);
    }
...
if (!featureName || !newStatus) {
  console.log('Usage: node updateSprint50Feature.js "Feature Name" "Status"');
  console.log('Example: node updateSprint50Feature.js "API Provider Registry Schema" "Done"');
  process.exit(1);
}
