#!/usr/bin/env node
/**
 * Update a Sprint 53 feature status in Notion
 * Usage: node updateSprint53Feature.js "Feature Name" "Status"
 */
...
async function updateFeature(featureName, newStatus) {
  try {
    ...
    if (response.results.length === 0) {
      console.log(`❌ Feature "${featureName}" not found in Sprint 53`);
      process.exit(1);
    }
...
if (!featureName || !newStatus) {
  console.log('Usage: node updateSprint53Feature.js "Feature Name" "Status"');
  console.log('Example: node updateSprint53Feature.js "Design chat interface" "Done"');
  process.exit(1);
}
